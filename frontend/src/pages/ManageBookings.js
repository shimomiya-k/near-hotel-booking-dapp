...
const ManageBookings = ({ isSignedIn, wallet, contract }) => {
  // 予約データを設定する
  ...
  const getBookedRooms = async () => {
    try {
      setBookedRooms(
        await contract.get_booking_info_for_owner(wallet.accountId)
      );
    } catch (error) {
      console.log({ error });
    }
  ...
  const handleCheckIn = async (room_id, check_in_date) => {
    // 部屋が空室かを確認
    let isAvailable = await contract.is_available(room_id);
    ...
    try {
      contract.change_status_to_stay(room_id, check_in_date).then((resp) => {
        getBookedRooms();
      });
    } catch (error) {
      console.log({ error });
  ...
  const handleCheckOut = async (room_id, check_in_date, guest_id) => {
    try {
      contract
        .change_status_to_available(room_id, check_in_date, guest_id)
        .then((resp) => {
          getBookedRooms();
        });
    } catch (error) {
